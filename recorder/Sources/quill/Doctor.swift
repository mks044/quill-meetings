...
enum DoctorReport {
    ...
    /// Never discover a missing dependency after an important meeting: check
    /// the whisper CLI and whether the model is already cached.
    static func checkTranscription() -> Check {
        ...
        guard WhisperEngine.cliPath() != nil else {
            return Check(
                name: "transcription",
                status: .fail("whisper-cli not installed"),
                remediation: "brew install whisper-cpp"
            )
        }
        if FileManager.default.fileExists(atPath: WhisperEngine.modelPath.path) {
            return Check(name: "transcription", status: .ok, remediation: nil)
        }
        return Check(
            name: "transcription",
            status: .warn("whisper model not downloaded (~1.6 GB)"),
